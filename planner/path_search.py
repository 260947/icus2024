...
class Planner:
    ...
    def path_searching(self,start,goal):
        ...
        ##while iters < self.max_iter:
            ...
            ########################################
            gradient_2 = att_force_2 + rep_force_2
            grad_len_2 = np.linalg.norm(gradient_2)
            gradient_2 = gradient_2 / grad_len_2  # 归一化
            vel_2 = max_vel * (1 - np.exp(-grad_len_2 / 5)) * gradient_2

            ###22处理局部极值22###
            attr_v_2 = att_force_2 / np.linalg.norm(att_force_2)
            rep_v_2 = rep_force_2 / (np.linalg.norm(rep_force_2) + 0.0001)
            cross_2 = np.cross(attr_v_2, rep_v_2)  #
            dot_2 = np.dot(attr_v_2, rep_v_2)

            if np.linalg.norm(curr_pos_2 - start) > 10 * self.goal_tol and dot_2 < -0.9 and (len(path_reverse) >= 3 and np.linalg.norm(path_reverse[-3] - path_reverse[-1]) <= 0.05):
                # path_reverse.pop()
                # path_reverse.pop()
                # path_reverse.pop()
                # curr_pos_2 = path_reverse[-1]
                ...
                min_distur_2 = (-dot_2 / (np.linalg.norm(attr_v_2)) ** 2) * rep_v_2 + attr_v_2  # 和斥速度垂直，与吸速度同一大致方向
                # min_distur = -min_distur #和斥速度垂直，与吸速度同一反方向
                min_distur_2[:] = np.linalg.norm(rep_force_2) * min_distur_2 / np.linalg.norm(min_distur_2)
                # min_distur = min_distur + 2*att_force
                print(min_distur_2)

                adjust_2 = 0
                if last_iter_2:
                    cos_2 = np.dot(last_distur_2, min_distur_2) / (
                                np.linalg.norm(min_distur_2) * np.linalg.norm(last_distur_2))
                    if cos_2 < -0.7 and iters - last_iter_2 < 10:
                        #min_distur_2 = -min_distur_2
                        min_distur_2 = last_distur_2
                        adjust_2 = iters - last_iter_2
                last_distur_2 = min_distur_2
                last_iter_2 = iters



                gradient_2 = att_force_2 + rep_force_2 + min_distur_2  ### 2 * np.linalg.norm(rep_force) * cross_/np.linalg.norm(cross_)
                grad_len_2 = np.linalg.norm(gradient_2)
                gradient_2 = gradient_2 / grad_len_2  # 归一化
                vel_2 = max_vel * (1 - np.exp(-grad_len_2 / 5 )) * gradient_2

                vel_2 = (1 + adjust_2/10) * vel_2
            ...
            rep_v = rep_force / (np.linalg.norm(rep_force) + 0.0001)
            cross_ = np.cross(attr_v, rep_v)  #
            dot_ = np.dot(attr_v, rep_v)

            if np.linalg.norm(curr_pos - goal) > 10 * self.goal_tol and  dot_ < -0.9 and (len(path)>=3 and np.linalg.norm(path[-3] - path[-1]) <= 0.05):  #or (len(path)>=3 and np.linalg.norm(path[-3] - path[-1]) <= 0.005)
                ...
                #u =
                min_distur[:] =  np.linalg.norm(rep_force) * min_distur/np.linalg.norm(min_distur)
                #min_distur = min_distur + 2*att_force
                print(min_distur)
                ...
                gradient =  att_force +  rep_force + min_distur ### 2 * np.linalg.norm(rep_force) * cross_/np.linalg.norm(cross_)
